...
**mission-control (root)** — the git repo at `Workspace/mission-control/`, remote `E0993599799/arigeo-hr.git`, branch `main`. This is the **deployed checkout**: its `.vercel/project.json` links it to Vercel project `prj_hfdIL1BYKitpZTxpH5EixAputdzQ` ("mission-control" in Vercel's UI). **Currently mid-merge-conflict**: `git status` shows 110 changed paths including an unresolved conflict (`pnpm-lock.yaml`, `tools/LAST_BACKUP_DIR.txt`, both deleted-by-us) left from an interrupted `pull --rebase --autostash`. The production-deployed branch is stuck, not merely behind.

**arigeo-hr (nested)** — `mission-control/arigeo-hr/`, a *second, separate clone* of the same `E0993599799/arigeo-hr.git` remote, nested one level inside the mission-control root's own working tree. 1 real commit ahead of root (2026-09-03, "feat(ess): employee self-service payslip view"), plus minor uncommitted edits. Root isn't simply "behind" it — root is separately stuck (see above); reconciling which copy is authoritative is a ticket, not yet decided.

**Omega — corrected, not a real anomaly** — `mission-control/Omega/` has no `.git` of its own; running git commands inside it silently resolved to the mission-control root (same lesson as `git -C <dir>` walking up to a parent repo when there's no `.git` present), which is why it first looked like an identical duplicate checkout. It's actually a plain subdirectory holding a **new, uncommitted dashboard feature** (`OMEGA_BUILD_PLAN.md`, `PHASE1_PROGRESS.md`, a Next.js app under `dashboard/`), staged since 2026-08-31, never committed — one more piece of the root's stuck state, not a separate mystery.

**mission-control-vercel-curated2** — resolved by ticket 03. Its `.git` looked like a real embedded repo but is actually **inert**: only 8KB (`HEAD` + `config`, no object database), a leftover from an interrupted `cp -r` of the Workspace repo's own `.git` — git silently walks up to the outer Workspace repo when read, same trap as Omega. Safe to remove (no history to lose). The **working tree itself is real and separate**, though: a scoped "salary-certificate" Next.js feature with its own `.vercel/project.json` pointing at a **third, distinct Vercel project and team** (`prj_cDTDRatZHXSthMwJ4g3AXBL4Rxyq` / `team_okNwqr3o8ikdFxEEpcckhPlC`) — neither mission-control root's nor anything else seen so far. Not safe to delete outright; needs reconciling into real version control if kept. See ticket 07 for the newly-surfaced question this creates.
...
**Orphaned worktree** — a `mission-control-<slug>` directory with a worktree-link `.git` file that is *not* in the root's current `git worktree list` output. Known cases: `mission-control-agent-2`, `mission-control-hermes-watcher`, `mission-control-issue234-0a` — all three **actively broken**, not just unregistered: their `.git` link files point at mismatched Windows/WSL path formats, so `git` fails with `fatal: not a git repository` inside them. Need `git worktree repair` or manual removal, not simple pruning.
