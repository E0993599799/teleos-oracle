...
**mission-control-vercel-curated2** — resolved by tickets 03 and 07. Its `.git` looked like a real embedded repo but is actually **inert**: only 8KB (`HEAD` + `config`, no object database), a leftover from an interrupted `cp -r` of the Workspace repo's own `.git` — git silently walks up to the outer Workspace repo when read, same trap as Omega. Safe to remove (no history to lose). The **working tree is load-bearing**: a "salary-certificate" Next.js feature confirmed **live** on `prj_cDTDRatZHXSthMwJ4g3AXBL4Rxyq` (พี่เอก's *personal* Vercel account — `team_okNwqr3o8ikdFxEEpcckhPlC`, distinct from the org account, `team_OS8nENECHPCuieeZsEhya9sF`, used everywhere else in this effort). Zero real version control currently protects it. See ticket 10 for giving it proper version control.
